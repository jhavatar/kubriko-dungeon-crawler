...
- **`plugin-dungeon-crawler`** — rendering layer, depends on `plugin-tilemap`. Key types:
  ...
  - `DungeonAtlas`: descriptor for a uniform tile atlas image — tile size, column count, and per-surface tile indices.
  ...
  - `DrawCommand`: sealed class — `FrontStrip` (front wall sub-interval), `SideStrip` (side-wall trapezoid), `FloorCeilingBand` (floor + ceiling pair with perspective quad corners for texture mapping).
